# cc-forge — Foundations Design (rationale edition)

> **Status:** final · this is the spec
> **Date:** 2026-05-22
> **Audience:** cc-forge maintainers
> **Reading time:** ~40 minutes
> **This edition** includes the full design rationale — the gap
> discoveries and the "why" behind each decision (sections marked
> `[INTERNAL]`). The public edition (`DESIGN.md`) is this document
> with `[INTERNAL]` sections stripped.
>
> This document incorporates two rounds of review (cc-forge CC for
> implementation feasibility, CLARK CC for reality-fit) and is the
> implementation brief for Session 0 (plugin conversion). No further
> review rounds are planned; where implementation reveals a flaw, this
> document is updated first, then the code follows.
...
### 2.1 The problem this section solves [INTERNAL]

cc-forge's current behavior:

- The closing banner pattern is defined in CLAUDE.md and HERMES.md.
- HERMES.md is not actually shipped to projects (lives only in cc-forge
  source).
- The `session-lifecycle/` directory is also not shipped to projects.
- The Stop hook is not registered in `~/.claude/settings.json`.
- A `stop.sh` script exists but is dead code — it only logs to
  `usage.log`.
- The SessionStart hook in `~/.claude/settings.json` is hardcoded to one
  specific project path.
- Zero `session_end` events have ever been logged.

The result: Hermes session-close fires only when the model happens to
attend to the CLAUDE.md instruction. It worked on 2026-05-21 because
the day was dominated by gate reviews (a salient context that reinforces
discipline) and decayed over subsequent sessions. The opening banner
exists today as a one-off hardcoded experiment in CLARK and doesn't
ship with cc-forge at all.

The initial fix proposed in early drafts was: move the bookends into
hooks, have the hook compute the content, have the model render it
verbatim. CLARK CC's review caught the flaw in this: *rendering hook
output is itself a model behavior.* The hook deterministically produces
the banner content and injects it into context, but whether the model
renders it verbatim is the same soft, advisory mechanism the design
claims to eliminate — just pushed one layer down. We would expect the
model to paraphrase, truncate, or skip rendering some non-trivial
fraction of sessions.

This realization reshaped the section. The fix is not "make rendering
deterministic" (impossible in an LLM-driven chat). The fix is to put
the framework's integrity in the deterministic side effects — the logs,
the dashboard data, the state files — and treat the rendered banner as
a high-reliability user-facing signal that the framework verifies after
the fact. See §2.4.

### 2.2 Where integrity lives [PUBLIC]
...
### 2.3 The bookend banners (the visible layer) [PUBLIC]
...
### 2.4 The hook ↔ model contract [PUBLIC]
...
### 2.5 Retrospective verification [PUBLIC]
...
### 2.7 SessionStart performance: the cache layer [PUBLIC]
...
### 2.8 Per-prompt framing (UserPromptSubmit) [PUBLIC]
...
### 2.10 What this section does NOT address [PUBLIC, brief]
...
### 3.1 The problem this section solves [INTERNAL]

cc-forge has had at least two distinct backlog item formats in
production. Multiple subsystems read and write these files — init, the
persona gate-review protocol, `/hermes-next`, `/hermes-status`,
`/hermes-dashboard`, `taskmaster-seed` — and none shared a contract.
RISKS.md has had at least two formats too. state.json has had
inconsistent field naming.

Together these are a class of bug: subsystems agree on what files they
share but disagree on what those files contain. Codifying contracts
eliminates the class.

The format choice is also a choice about *who writes these files*. The
persona update protocol writes backlog items via line-anchored edits.
That makes block-format (one item per multi-line section) the right
choice — personas can reliably edit a specific line within a block.
Pipe-table format would require recomputing column widths on every
edit, which is error-prone for the actual writers even if it's compact
for one consumer (the parser). The format follows the writers, not the
single parser. (An earlier draft chose pipe-tables; cc-forge CC's
review correctly identified this as optimizing for the wrong party.)

### 3.2 Backlog items [PUBLIC]
...
**CCA-F cross-check for this decision [INTERNAL]:** see the CCA-F
Cross-Check section, finding **C-1** — why we declined to defend the
classifier as a special case and added the deterministic backstop
instead.
...
### 3.8 Failure behavior — strict vs advisory [PUBLIC]
...
### 3.9 Schema migration — all formats [PUBLIC]
...
### 3.10 What this section does NOT address [PUBLIC, brief]
...
### 4.1 The problem this section solves [INTERNAL]

Three structural issues in current cc-forge: no plugin manifest (commands
installed by shell script into `~/.claude/commands/`, enabling gap #50
and #51); hooks not standardized (SessionStart hardcoded to one path, no
Stop hook); and `/hermes-update` with no schema for what to copy (every
new file type requires manual update-list changes — gap #52). The
three-layer split with explicit ownership eliminates all three.

### 4.2 The three layers [PUBLIC]
...
### 4.3 Layer 1 — Plugin structure [PUBLIC]
...
### 4.4 Layer 2 — Project state [PUBLIC]
...
### 4.5 Layer 3 — User-maintained [PUBLIC]
...
### 4.6 The `/hermes-update` contract [PUBLIC]
...
### 4.7 The Session 0 migration (CLARK first) [PUBLIC]
...
### 4.8 What this section does NOT address [PUBLIC, brief]
...
### 5.1 The problem this section solves [INTERNAL]

Every gap caught in two weeks was observable from project state alone —
corrupted command files (list the dir), missing scripts (check
existence), zero session_end events (grep), dashboard zeros on a
populated backlog (run the parser), missing Standard fields (grep). None
needed clever analysis. They needed someone to *look*. The doctor is the
framework looking at itself, on demand, with a script.

### 5.2 What the doctor does [PUBLIC]
...
### 5.3 The check catalogue [PUBLIC]
...
### 5.4 When the doctor runs [PUBLIC]
...
### 5.5 What the doctor can fix [PUBLIC]
...
### 5.6 Implementation notes [PUBLIC]
...
### 5.7 What this section does NOT address [PUBLIC, brief]
...
### 6.1 Deferred to subsequent sessions [PUBLIC]
...
### 6.9 What this design does NOT solve (acknowledged) [PUBLIC]
...
### 7.2 Near-term horizontal growth [PUBLIC]
...
### 7.5 The deliberate non-roadmap [PUBLIC]
...
### 7.6 How this roadmap evolves [PUBLIC]
...
**Tagging:** `[INTERNAL]` = rationale, stripped from the public edition.
`[PUBLIC]` = appears in both. Untagged = public by default.
...
## CCA-F Cross-Check [INTERNAL]

A section-by-section cross-check of this design against the Claude
Certified Architect — Foundations (CCA-F) curriculum. This section is
internal working reference only; it never appears in the public
`DESIGN.md`. The design itself is stated in cc-forge's own terms in the
sections above; this section records *why we examined each area, what
CCA-F says, and what we decided* — including divergences we keep.

**Method.** For each design area, three questions: (1) what does CCA-F
call this pattern (vocabulary alignment); (2) are we doing it the
canonical way or diverging defensibly (correctness); (3) are there
CCA-F-canonical things here we missed (gap check). Findings are tagged
Category A (vocabulary alignment), B (tacit pattern made explicit), or
C (genuine gap).

**Curriculum version.** Cross-check run against the live CCA-F domain
pages as of 2026-06. Note a weight drift since the previous-thread
digest (entry #22): the live domains index now lists D1 Agentic
Architecture at ~27% (digest recorded 25%) and D2 Tool Design & MCP at
~18% (digest recorded 20%). Weights are not load-bearing for this work;
recorded for traceability.

### §2 — Session lifecycle (CCA-F D1)

**C-1 — Intake classifier vs the model-confidence anti-pattern
(Category C, resolved).** CCA-F D1 (Hooks & Programmatic Enforcement)
lists "self-reported confidence scores for escalation decisions" as an
enforcement anti-pattern: model confidence is not a reliable gate for a
critical rule, and the canonical fix is deterministic programmatic
enforcement. The intake classifier (§3.6) is a confidence-thresholded
model call, so on its face it touches this anti-pattern.

*What we decided.* Not to defend the classifier as a special case —
even though it is arguably different in kind from the anti-pattern (a
separate classification call on third-party user input, vs an agent
self-assessing its own task under incentive to continue) — but to
refuse to *rely* on that distinction. Per §2.2, integrity must live in
deterministic side effects. So the classifier is demoted to a
probabilistic first line, and a deterministic backstop is added: the
`intake_reconciliation` doctor check (§5.3), which flags any backlog
scope that entered without a corresponding `intake_step`. This is
strictly more robust than either the classifier alone or a defended
divergence, and keeps the framework consistent with its own central
principle. The named residual gap — bypassed scope that leaves no
structural trace at all — is recorded in DESIGN §3.6 rather than assumed
away, in keeping with the §6.9 house style. User confirmed approach (b):
keep the cheap classifier as first line, add the deterministic backstop.

**C-2 — Stale context vocabulary (Category A + small B, resolved).**
CCA-F D1.4 (Session Management) names "stale context detection and
mitigation in long-running sessions" as a core concept. Two observations
fall out of comparing this design to that vocabulary:

- Our §6.9 "mid-session state changes between bookends" is the *same
  phenomenon* CCA-F calls stale context — long-session drift where data
  surfaced early goes out of date. Same problem, different name. Recorded
  here for the cross-check; the §6.9 public prose is left in cc-forge's
  own terms (the observation that the two are identical is the
  cross-check's, not the design's, so it does not belong in the public
  spec).
- Our `cache.json` mtime-invalidation (§2.7) is a stale-context
  mitigation that was previously unlabeled. Resolution: §2.7 now states
  the staleness property explicitly ("a freshness-checked read, not a
  time-to-live guess") in cc-forge's own terms.

No implementation change — the mtime logic was already specified; this
was labeling and a cross-check note.

**C-3 — `fork_session` has no analog (Category C / non-goal, resolved).**
CCA-F D1.4 treats session forking (`fork_session`, branched exploration
without context pollution) as a core session operation. cc-forge has no
branching concept. This is correct for a single-developer SDLC framework
(single linear session history, append-only `usage.log`, single current
state snapshot), but the absence should be visible as a *choice*.
Resolution: §6.8 ("Deliberate silence") now names session branching as a
deliberate non-goal in cc-forge's own terms, so a CCA-F-literate reader
does not read the absence as an oversight. No implementation change — a
non-goal, by definition.

*Deliberate non-match (no action).* CCA-F D1.1 (Agentic Loops,
stop_reason loop control) governs the agent execution loop, which is
Claude Code's responsibility, not cc-forge's — the framework does not
author the `while stop_reason == "tool_use"` loop. Recorded as a
considered non-match, not an oversight.

### §3 — File contracts (CCA-F D2/D4)

**Vocabulary alignment (Category A).** §3's parser contracts are
structured-output schema validation in CCA-F terms (D4.3): required vs
optional fields, enum fields, strict schemas. The strict/advisory split
(§3.8) maps onto D2.2 structured error responses (errorCategory,
isRetryable) — generic failures hidden vs structured metadata that
enables recovery. Partial-parse safety ("N of M parseable," denominator
before percentages) is the operational form of D2.2's "uniform generic
failures prevent appropriate recovery." Note the *inversion*: D4.3 is
about constraining *Claude's outputs* via tool_use so they are
schema-valid at emission; cc-forge validates *markdown files personas
wrote* retrospectively. Same discipline, opposite application point.

**D-1 — Retrospective validation vs emission-time guarantee, plus the
missing retry half (Category C, resolved).** CCA-F D4.3's canonical
answer to "reliable structured data from a model" is tool_use with a
JSON schema — structure guaranteed at emission. cc-forge instead lets
personas write markdown and validates strictly after the fact.

*Divergence kept and defended.* The tool_use answer assumes the
structured output is consumed *programmatically*. cc-forge's backlog and
intake files are human-editable Layer 2/3 artifacts — markdown a user
reads, edits, and commits to git. Emitting via tool_use JSON would make
JSON the canonical form and destroy the "markdown in git that humans
review" property that is core to the framework's value. Different
constraint, defensible divergence. Retrospective markdown validation
stays.

*The genuine gap, now closed.* CCA-F D4.4 (validation-retry) *does*
apply and was missing. cc-forge had the validation half (strict parser,
violation logging) but not the retry-with-feedback half: a malformed
persona write was logged and skipped, left in place until a human or the
doctor noticed. D-1 adds the missing half — §3.8 now specifies that a
structured persona write is validated at write time and, on failure, the
specific violation is fed back to the persona for self-correction before
the write completes. This mirrors C-1 (§2): the framework had
deterministic detection, was missing the correction loop. Retryable
(persona write) vs non-retryable (absent file, hand-edit) is the D2.2
isRetryable distinction; the D4.4 retry-limit (cannot conjure absent
information) is stated explicitly. Per user direction, the principle and
contract surface are captured here; detailed mechanics defer to Session
D, which owns `backlog-update-protocol.md` — same pattern as §2.8.
*Implementation surface:* the write protocol gains a validate-retry step.

**Enum extensibility (Category C, partial — one field changed).** CCA-F
D4.3 recommends `"other" + detail` / `"unclear"` escape values so a
model is not forced to fabricate a fit for a closed enum. Reviewed all
three enums. `Status` stays closed — it is a state machine, "other" is
nonsensical for a state. `disposition` stays closed — procedural, and
`intake-pending` status already covers in-triage ambiguity. Only
`classification` (`feature|bug|improvement|spike`) is genuinely
model-filled judgment where "fits none of these" is plausible; it gains
`other` + required `classification_detail` (§3.7). Decision recorded so
the two unchanged enums are on record as considered-and-kept-closed, not
overlooked.

### §5 — `/hermes-doctor` (CCA-F D5/D3)

**Vocabulary alignment (Category A).** The doctor is a reliability
instrument (D5). Its `--json` mode + exit codes are D3.6 CI integration.
"Same parsers as the dashboard, one canonical parser per format" (§5.6)
is the structural fix for the format-fork gap and embodies D5's
consistency/provenance discipline — sound, unchanged.

**Deliberate non-match (recorded, no action).** D5.5 is largely about
field-level *confidence scores* and *calibrating review thresholds*. A
CCA-F reviewer will look for the doctor's confidence calibration and not
find it — correctly. The doctor's checks are deterministic predicates (a
file parses or it does not; `session_end` is present or absent), so
there is no probabilistic judgment to calibrate. Confidence calibration
applies to model-judgment outputs, not deterministic checks. The one
exception is the banner-rendering check (fuzzy transcript match), which
is why E-3 below adds a measurement-error caveat to that single check.

**E-1 — Aggregate metrics mask per-segment failure (D5.5, Category C,
resolved).** D5.5's central warning — "aggregate accuracy may mask poor
performance on specific segments" — applies directly. "2/10 banner
misses" hides whether misses cluster on post-compact resumes; "150/200
parse" hides whether failures are one broken file or systemic. The raw
data already exists (session-start matcher is recorded; §3.8 counts
per-file violations), so this is a *reporting* gap, not a collection
gap. Resolution: §5.3 now reports two dimensions stratified — banner-miss
by session-start type, format violations by file/domain — scoped tightly
to where the segment is actionable (per user direction; not a general
"stratify everything" principle). This is the gap the prior thread
flagged as "no stratified dashboard metrics," confirmed against D5.5 and
resolved for both doctor and dashboard. *Implementation surface:*
`hermes-doctor.py` output logic; dashboard breakdown.

**E-2 — `--json-schema` for CI (D3.6, Category B, resolved).** D3.6
names `--json-schema` for machine-parseable, schema-valid CI output. The
doctor had `--json` (format) but no declared schema for it. Resolution:
§5.6 now specifies the `--json` output conforms to a declared, versioned
schema, with the version in the output so consumers detect shape drift —
the doctor eating its own dogfood (a validated output contract). Ties to
D-1: the doctor's own output is structured output that deserves a
schema. *Implementation surface:* a JSON schema file shipped with the
plugin.

**E-3 — Honesty caveat on the one approximate check (D5.5, Category B,
resolved).** The banner-rendering sample check is the only
non-deterministic doctor check (fuzzy transcript match). D5.5 discipline:
an approximate measurement should declare its own error. Resolution:
§5.3 now notes the banner check reports its rate as approximate and that
every other check is a definite pass/fail. One-sentence caveat,
consistent with §6.9 house style. No mechanism change.

### §4 — Physical layout (CCA-F D3)

**Vocabulary alignment (Category A).** §4 is the Claude Code config
architecture (D3). The three-layer model relates to D3's config
hierarchy/scoping but on a different axis: cc-forge's layers are an
*ownership/writability* axis (who may edit), D3.1's hierarchy is a
*precedence/scope* axis (user/project/directory). Related, not
identical. The gap #51 fix (drop global `~/.claude/commands/`, plugin
project-local only) is D3.2's project-scoped vs user-scoped command
discipline, cleanly aligned. Plugin command namespacing goes beyond the
D3.2 curriculum (which covers raw `.claude/commands/`).

**`@import` boundary clarified (defensible divergence, resolved).** D3.1
endorses `@import` as a canonical modularity mechanism; §4.5 dropped
`@import`. Surface contradiction, but the two address different things:
D3.1's `@import` is *user-authored modularity* (a user splitting their
own CLAUDE.md); cc-forge dropped *framework-mandated* `@import` (a
framework file the user's CLAUDE.md was required to import, which fails
open). Resolution: §4.5 now draws the boundary explicitly — framework
never depends on a Layer 3 import; user `@import` for their own
modularity is untouched. No conflict with D3.1 once the distinction is
drawn.

**F-2 — `context: fork` for verbose/parallel operations (D3.2, Category
C, resolved, user-requested detail).** D3.2's `context: fork` answers
exactly cc-forge's known token-cost problem (gate reviews = highest
token activity). Grounded against the official Claude Code skills docs:
a forked skill runs in a fresh sub-agent with its own context window,
receives only its own instructions (not conversation history), does
verbose work in isolation, and returns only its result. Resolution:
§4.3 specifies `/hermes-doctor`, `/hermes-dashboard`, and parallel gate
reviews declare `context: fork`, with two load-bearing qualifications
discovered during grounding: (1) *precondition* — only operations
self-contained from Layer 2 state are forkable, which is §2.2's
principle validating itself (an operation whose integrity lives in state
can run in a context that never saw the session); (2) *fork is
optimization, not correctness* — a known Claude Code bug silently runs
forked skills inline, so cc-forge must stay correct whether fork happens
or not, never depending on it. The second qualification keeps cc-forge
consistent with its own "don't depend on what fails open" doctrine.
*Implementation surface:* `context: fork` frontmatter on the relevant
command/skill files; gate-review parallelism.

**F-3 — `allowed-tools` scoping (D3.2, Category B, resolved).** D3.2
names `allowed-tools` frontmatter to restrict command capability.
cc-forge commands didn't declare tool restrictions. Resolution: §4.3
specifies commands declare `allowed-tools` (read-only commands get a
read-only set; mutating commands declare only what they need) — same
least-privilege conservatism as the doctor's `--fix` discipline.
*Implementation surface:* frontmatter on command files.

**F-1 — `.claude/rules/` path-specific rules (D3.1/D3.3, Category C,
declined as non-goal).** D3.3 is entirely about `.claude/rules/` with
glob frontmatter (`paths: [...]`) loading conventions conditionally by
which file is being edited. cc-forge has no analog, deliberately:
cc-forge's discipline is *phase/persona-scoped* (what phase, which
persona is acting), not *file-path-scoped* (which file is open).
Path-specific rules are a code-convention mechanism; cc-forge operates
at the SDLC-process layer above that. Recorded as a considered non-goal
so the absence reads as a choice, not an oversight. User agreed.

### §6 / §7 / appendices

**§6.9 (unsolved problems) — checked, no new rows.** Verified whether
anything surfaced in the cross-check is genuinely *unsolved* and belongs
in §6.9. Nothing does: C-2 stale context is solved (cache freshness
check; the existing mid-session-drift row already covers the residual);
the fork silent-inline mode is *handled* (correctness independent of
fork), not open; D-1 and classifier residuals are named in-place (§3.8,
§3.6). §6.9 unchanged.

**§7.2 Code Reviewer — left as-is.** D4.6 and D3.6 both back the
"independent instance reviews code it didn't write" property, and the
fork mechanism (F-2) now provides it. But the Code Reviewer is already
on the roadmap; sharpening its rationale is polishing existing prose, so
per the keep-as-is rule it is left unchanged. The independence property
is recorded here for whoever specs it.

**Roadmap additions (CCA-F patterns neither built nor previously
roadmapped).** Applying the rule "CCA-F-canonical and not yet
included → roadmap":

- **MCP resource exposure (D2.4) → §7.3.** cc-forge has no MCP surface.
  Added as a medium-term direction: expose Layer 2 state (backlog,
  risks, decisions, phase) as read-only MCP *resources*. Authoring full
  MCP servers remains out. Confirms the prior-thread "no MCP" Category C
  gap.
- **Plan mode (D3.4) → §7.3.** cc-forge doesn't invoke Claude Code plan
  mode. Added as a medium-term item to evaluate plan mode for high-bar
  architectural transitions, complementing gate reviews. Whether it adds
  value over existing gate discipline is the open question the item
  exists to settle. Confirms the prior-thread "no plan mode" Category C
  gap.

**Crash-recovery manifest (D5.4) — already covered, not roadmapped.**
D5.4's "structured state persistence for crash recovery" is substantially
what §2.2 already provides: integrity lives in side effects, so an
interrupted session's state is reconstructable from Layer 2 (state.json,
usage.log, handoff side-effects). Recorded as covered-by-existing-design
rather than a gap.

**Deliberate non-matches (recorded across the cross-check).** D1.1
stop_reason loop (Claude Code owns the loop); D1.7 fork_session as a user
operation (C-3); D3.3 `.claude/rules/` path-scoped rules (F-1); D5.5
confidence calibration on deterministic checks (§5). Each is a CCA-F
pattern cc-forge deliberately does not adopt, recorded so absences read
as choices.

**Consistency:** Appendix B gap→check mapping unchanged except the gap
#49 row (C-1); E-1 stratification is a D5.5 improvement, not gap-driven,
so it adds no Appendix B row. Appendix D gains three mechanism rows
(intake enforcement, structured write integrity, verbose/parallel ops).

---

**Cross-check status: complete.** All sections (§2–§7, appendices)
cross-checked against CCA-F D1–D5. Findings: C-1, C-2, C-3 (§2); D-1 +
enum (§3); E-1, E-2, E-3 (§5); F-1, F-2, F-3 (§4); two roadmap additions
(§7). Curriculum version: live CCA-F domain pages + official study-guide
subdomain breakdown, 2026-06.
