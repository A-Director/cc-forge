...
if [ -f "${PROJECT_ROOT}/.gitignore" ]; then
  ...
  for line in \
    ...
    "# (.cc-forge/cache.json is the freshness-checked Layer-2 cache — local-only, regenerated by /hermes-doctor)" ; do
...
# (status/argus-last-run.md and other markdown reports stay committed)
# (.cc-forge/usage.log is committed — it's the project's event history)
# (.cc-forge/cache.json is the freshness-checked Layer-2 cache — local-only, regenerated by /hermes-doctor)
EOF
