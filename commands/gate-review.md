...
description: >
  Trigger an SDLC gate review. Reads the current stage, determines which
  personas are due, invokes them as forked subagents, and produces a
...
- Standard: [if applicable, else —]
- Context: [Why this decision was needed — from the gate review finding]
- Decision: [What was decided — specific and concrete]
- Rationale: [Why this option — from the persona's reasoning]
- Alternatives considered: [Option] (rejected because [reason]); [Option] (rejected because [reason])
